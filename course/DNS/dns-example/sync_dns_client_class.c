...
//dns_async_client_proc(void *arg)
//epoll_wait
//result --> callback
...
struct async_context* dns_async_client_init(void) {
	...
	if (ctx == NULL) {
		printf("dns_async_client_init calloc failed\n");
		close(epfd);
		return NULL;
	}
	ctx->epfd = epfd; //
	...
	if (ret) {
		printf("dns_async_client_init pthread_create failed\n");
		close(epfd);
		free(ctx);
	}
...
int dns_async_client_commit(struct async_context *ctx, const char *domain, async_result_cb cb) {
	...
	struct sockaddr_in dest; //dns server addr
	bzero(&dest, sizeof(dest));
	dest.sin_family = AF_INET;
	dest.sin_port = htons(53);
	dest.sin_addr.s_addr = inet_addr(DNS_SVR);
	...
	ev.data.ptr = eparg;
	ev.events = EPOLLIN;
	printf("dns_async_client_commit\n");
