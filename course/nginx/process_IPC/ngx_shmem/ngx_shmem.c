...
int shm_alloc(shm_t *shm)
{
    ...
    if (shm->addr == MAP_FAILED) {
        fprintf(stderr, "mmap(MAP_ANON|MAP_SHARED, %lu) failed, reason: %s.\n", shm->size,strerror(errno));
        return NGX_ERROR;
    }
...
void shm_free(shm_t *shm)
{
    if (munmap((void *) shm->addr, shm->size) == -1) {
        fprintf(stderr, "munmap(%p, %lu) failed, reason: %s.\n", shm->addr, shm->size, strerror(errno));
    }
}
...
int shm_alloc(shm_t *shm)
{
    ...
    if (fd == -1) {
        fprintf(stderr, "open(\"/dev/zero\") failed.\n");
        return NGX_ERROR;
    }
    ...
    if (shm->addr == MAP_FAILED) {
        fprintf(stderr, "mmap(/dev/zero, MAP_SHARED, %lu) failed, reason: %s.\n", shm->size, strerror(errno));
    }

    if (close(fd) == -1) {
        fprintf(stderr, "close(\"/dev/zero\") failed.\n");
    }
...
{
    if (munmap((void *) shm->addr, shm->size) == -1) {
        fprintf(stderr, "munmap(%p, %lu) failed", shm->addr, shm->size);
    }
}
...
int shm_alloc(shm_t *shm)
{
    ...
    if (id == -1) {
        fprintf(stderr, "shmget(%lu) failed, reason: %s.\n", shm->size, strerror(errno));
        return NGX_ERROR;
    }
...
{
    if (shmdt(shm->addr) == -1) {
        fprintf(stderr, "shmdt(%p) failed, reason: %s.\n", shm->addr, strerror(errno));
    }
}
