...
#define NTY_DB_SELECT_NEW_ITEM			"select * from news where id='%s';" //41d20b131a90ac40-2860c1e3407b54e0

static int ntyQueryMartinNewsSelect(void *self, const char* id) {
	...
	SnailsSet r = ntyConnPoolExecuteQuery(con, NTY_DB_SELECT_NEW_ITEM, id);
	if (r != NULL) {
		while (ntyShrimpStmtResultSetNext(r)) {
			const char *key = ntyResultSetgetString(r, 1);
			const char *title = ntyResultSetgetString(r, 2);
			const char *text = ntyResultSetgetString(r, 3);
			const char *url = ntyResultSetgetString(r, 4);

			ntylog(" id : %s, name:%s, lnglat:%s, num:%s\n", key, title, text, url);
			ret = 0;
		}
	}
...
int main() {
	...
	ntyQueryMartinNewsSelect(pool, "41d20b131a90ac40-2860c1e3407b54e0");
